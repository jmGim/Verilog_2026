...
module tb_fsm_pattern();
    ...
    // 값이 변하면 값을 출력한다. 
    initial begin 
        $monitor("time = %t state = %b , in = %b, out = %b", $time, u_fsm_pattern.current_state, in, out);  // tb에 없고 instance안에 있는 변수 출력
    ...
    initial begin
       ...
       // #3 010111 out 1이 되면 S1
        ...
        @(posedge clk); in = 1; // S6 → S1 101011 검출
        #100 ;

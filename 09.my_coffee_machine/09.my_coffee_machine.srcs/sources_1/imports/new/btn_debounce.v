...
module btn_debouncer #(parameter DEBOUNCE_LIMIT = 1_000_000)
(
    input clk, reset,
    input [2:0] btn,
    output [2:0] debounced_btn
);
    // 하위 모듈에 파라미터를 반드시 전달해야 함 
    debouncer #(.DEBOUNCE_LIMIT(DEBOUNCE_LIMIT)) U_debouncer_btnL (
        .clk(clk), .reset(reset), .noisy_btn(btn[0]), .clean_btn(debounced_btn[0])
    );
    debouncer #(.DEBOUNCE_LIMIT(DEBOUNCE_LIMIT)) U_debouncer_btnC (
        .clk(clk), .reset(reset), .noisy_btn(btn[1]), .clean_btn(debounced_btn[1])
    );
    debouncer #(.DEBOUNCE_LIMIT(DEBOUNCE_LIMIT)) U_debouncer_btnR (
        .clk(clk), .reset(reset), .noisy_btn(btn[2]), .clean_btn(debounced_btn[2])
    );
endmodule
...
module debouncer #(parameter DEBOUNCE_LIMIT = 4'd10) (     // test bench : 20'd999   <-> 999_999
    input      clk,
    input      reset,
